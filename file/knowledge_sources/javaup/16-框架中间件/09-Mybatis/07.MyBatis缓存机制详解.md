...
description: "一级缓存SqlSession级别默认开启;二级缓存namespace级别配置;PerpetualCache HashMap实现;缓存命中条件与失效时机;update/insert/delete清空缓存;分布式环境缓存一致性问题。"
keywords: ["一级缓存", "二级缓存", "PerpetualCache", "namespace缓存", "缓存失效"]
---
