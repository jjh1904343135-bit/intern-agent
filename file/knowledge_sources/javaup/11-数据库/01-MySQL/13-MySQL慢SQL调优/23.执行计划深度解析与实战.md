...
description: "EXPLAIN命令获取执行计划;id/select_type/table/type/key/rows/Extra字段详解;type访问类型排序(system/const/eq_ref/ref/range/index/ALL);Extra关键提示信息;EXPLAIN ANALYZE实际执行统计。"
keywords: ["EXPLAIN", "执行计划", "type访问类型", "Extra字段", "SQL分析"]
---
...
执行上述语句后，MySQL返回如下结构的结果集（共12个字段）：

```
+----+-------------+-----------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table     | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-----------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | PRIMARY     | orders    | NULL       | ALL  | NULL          | NULL | NULL    | NULL | 1000 |   100.00 | Using where |
|  2 | SUBQUERY    | customers | NULL       | ALL  | NULL          | NULL | NULL    | NULL |  500 |   100.00 | Using group |
+----+-------------+-----------+------------+------+---------------+------+---------+------+------+----------+-------------+
```
...
|------|------|--------|
| id | 执行顺序标识 | 中 |
| select_type | 查询类型（SIMPLE、PRIMARY、SUBQUERY等） | 中 |
| table | 当前操作的表 | 低 |
| partitions | 匹配的分区 | 低 |
| **type** | 访问类型（ALL、index、range、ref、const等） | 高 |
| possible_keys | 可能使用的索引 | 中 |
| **key** | 实际使用的索引 | 高 |
| key_len | 使用的索引字节长度 | 中 |
| ref | 索引比较的列或常量 | 低 |
| rows | 预估扫描行数 | 高 |
| filtered | WHERE条件过滤百分比 | 中 |
| **Extra** | 附加信息（Using index、Using filesort等） | 高 |
