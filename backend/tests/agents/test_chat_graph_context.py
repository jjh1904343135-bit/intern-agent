...
from app.agents.chat_graph.context import ChatContextBudget, estimate_tokens, maybe_compress_context


def test_context_budget_triggers_compression_at_eighty_percent_and_keeps_recent_history() -> None:
    history = [
    ...
    result = maybe_compress_context(
        ...
        budget=ChatContextBudget(context_window_tokens=1200, compression_ratio=0.8, reserved_output_tokens=100),
    )
...
def test_context_budget_skips_compression_below_eighty_percent() -> None:
    ...
    result = maybe_compress_context(
        ...
        tool_context={},
        prompt="短提示词",
        budget=ChatContextBudget(context_window_tokens=8192, compression_ratio=0.8, reserved_output_tokens=900),
    )
